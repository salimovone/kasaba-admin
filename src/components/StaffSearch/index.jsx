...
const StaffSearch = () => {
  ...
  return !clicked ? (
    ...
    >
      ...
      <div className="w-3/4 border border-sky-300 rounded-xl py-8 -mt-44 px-4 relative z-50">
        ...
        <div className="w-full grid grid-cols-1 mt-5 gap-y-1 transition-all duration-150">
          {data.map((itm) => (
            <StaffElement nonLoader={true} {...itm} />
          ))}
        </div>
      </div>
    </div>
    ...
    <button
      onClick={() => setClicked((prev) => !prev)}
      className="border border-sky-300 w-6 h-6 px-[4px] py-[2px] rounded-lg mb-2 flex-center text-slate-700 cursor-pointer hover:bg-white"
    >
      <AiOutlineSearch />
    </button>
  );
